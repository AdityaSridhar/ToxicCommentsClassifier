...
class MeanEmbeddingVectorizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        y = [word_tokenize(sent) for sent in X]
        return np.array([
...
print('Done')
print('Accuracy of the model is {0}'.format(accuracy_score(y_test, pred)))
print('Classification report for the model: {0}'.format(classification_report(y_test, pred, target_names=levels)))
print('Hamming Loss for the model is {0}: '.format(hamming_loss(y_test, pred)))
